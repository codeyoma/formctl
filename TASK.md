...
- [x] Add opt-in protected artifact storage for sensitive local runs.
- [x] Keep agents reporting artifact paths rather than embedding artifact contents in chat.
- [x] Verify: cleanup removes expired runs.
- [x] Verify: protected artifacts are not readable without the configured key or passphrase.
- [x] Record usability tradeoffs in `REVIEW.md`.
