...
Manual recording labels text input as `input`, select controls as `select`, file inputs as `file`, and named non-submit button clicks as `click` so the YAML is easier to review.
